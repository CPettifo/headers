# Headers
## Overview
Headers is a full-stack web application that collects and displays information about a visitor’s IP address, geolocation, and device. It is hosted on a Raspberry Pi and exposed securely to the internet via a Cloudflare Tunnel, accessible at [headers.pettifor.tech](headers.pettifor.tech)

The system captures and processes request headers from users, enriches them with geolocation & security info (using the free vpnapi.io api (while it is available)), abstracts user agent details into categories (device, OS, browser), and displays this information back to the user through a simple web interface.
...
### Infrastructure
Deployed on a Raspberry Pi using Docker for each microservice & Docker Compose for orchestration
